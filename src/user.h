...
public:
	...
	map<string, vector<vector<ofPoint>>>animations;
	double frameNum = 0;
	int pFramenum = 0;
	void setupAnimation(string file, string name) {
		vector < vector<ofPoint>> p;
		p.resize(1);
		int indx = 0;
		ofBuffer buffer = ofBufferFromFile(file);
		for (auto line : buffer.getLines()) {
			if (line != "next pose") {
				vector<string> s = ofSplitString(line, " ");
				ofPoint po(std::stof(s[0]), std::stof(s[1]));
				// cout << po.x << " " << po.y << endl;
				p[indx].push_back(po);
			}
			else {
	...
	bool playAnimation(string name, float speed, bool loop = true) {
		frameNum += (ofGetLastFrameTime() * speed);
		int f = floorf(frameNum);
		

		if (pFramenum != f && f < animations[name].size()) {
			for (int i = 0; i < animations[name][f].size(); i++) {
				addPoint(i, animations[name][f][i]);
			}
			pFramenum = f;
		}
		...
		if (f > animations[name].size() && !loop) {
			return false;
		}
		else if (f > animations[name].size() && loop) {
			frameNum = 0;
			return true;
		}
	}
